...
{
	u8InputID = 0u;        /* prevent compiler warning */
    *u32PulseCount = 0u;   
    /* TODO: to be implemented if necessary */
}
BSP_StdReturnType BSP_ReportError_PulseCounter(uint8 u8InputID, uint8* FTB)
{
	u8InputID = u8InputID;  /* prevent compiler warning */
	*FTB = 0;               /* prevent compiler warning */
     /* TODO: to be implemented if necessary */
}
...
{
	FOutputId = FOutputId;     /* prevent compiler warning */
	u8DutyCycle = u8DutyCycle; /* prevent compiler warning */
     /* TODO: to be implemented if necessary */
}
...
BSP_StdReturnType BSP_Get_Frequency(uint8 FInputId, uint32 *Value)
{
	...
	{
		if(u8t_countingMethodStage == 0u)
		{
			return BSP_INVALID_RANGE;
		}		
		u32t_Freq_Input = (u32g_FreqCntValueInCounting <<8)*20u/u8t_countingMethodStage;/* convert the counter value to frequency */
...
/*
 * Name        : BSP_Get_PulseCounter
 * Description : get the pulse number by converted from the counter number
 * Arguments   : FInputId ---- frequency input capture ID
 *               *Value   ---- the pulse num value captured
 * Return      : BSP_OK
 */
BSP_StdReturnType BSP_Get_PulseCounter(uint8 FInputId, uint32 *Value)
{
	FInputId = FInputId;
	
    *Value = u32g_PulseCounterValue;				/* pointer points to the pulse num value */
...
{
	uint32 u32t_Freq_Cnt,u32t_Freq_Cnt_Decimal,u32t_Freq_Input_Integer, u32t_Freq_Input_Decimal, u32t_Freq_Input;
