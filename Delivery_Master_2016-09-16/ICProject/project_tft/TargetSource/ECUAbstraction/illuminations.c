...
#define ILLUMINATIONLEVELNUM 6u
#define ILLUMINATIONPERCENT (100u / (ILLUMINATIONLEVELNUM - 1u))
...
void BSP_Set_Illumination(uint8 ChannelId, uint8 IllumLevel)  
{
	...
	{
		SIU.PCR[LCD_BL].R = 0x4200;               /* set TFT_PWM as output, to be moved to rappid */
		SIUL_Output(LCD_BL, 1u);
	}
	else if(ChannelId == 0u)                        /* gauge illumination */
	{
        ...
        u16t_HDuration = (u16g_DispTbl[u8t_LevelIndex + 1] - u16g_DispTbl[u8t_LevelIndex]) * (uint16)(IllumLevel - ILLUMINATIONPERCENT * u8t_LevelIndex) / ILLUMINATIONPERCENT
