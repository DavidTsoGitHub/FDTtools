...
static uint32  u32g_FreqInputDataArray[20];    			 /* Freq Signal Input Data Buffer */
...
void MCU_FreqInput_Init(uint8 u8t_CurSel) 
{
    ...
    {
    	/* channel 16 used to be a up-counter ,frequency is 1M ,PLL0/64*/
...
void FreqInputCapTimerISR (void)
{
	...
	u8g_FreqInputDataArray_Counter = u8g_FreqInputDataArray_Counter % u8t_countingMethodStage;
	
	u32g_FreqInputDataSum_InArray = 0;                                     	/* initialize the Sum of Freq Signal Input Data Value in array */
	for(uint8 u8t_loop_flag=0; u8t_loop_flag<u8t_countingMethodStage; u8t_loop_flag++)   /* do a loop to get the sum of frequency count value in the Freq Input Data Buffer*/
	{
    ...
    if(u8g_FreqInputFlag_InCntMethod == 1)
	{
		...
		u32g_FreqInputDataSum_InArray = 0;                                     /* initialize the Sum of Freq Signal Input Data Value in array */
		for(uint8 u8t_loop_flag=0;u8t_loop_flag < FREQINPUTDATABUFFERSIZE;u8t_loop_flag++)   /* do a loop to get the sum of frequency count value in the Freq Input Data Buffer*/
		{
		    u32g_FreqInputDataSum_InArray +=u32g_FreqInputDataArray[u8t_loop_flag];	
		}
		u32g_FreqCntValueInCounting = u32g_FreqInputDataSum_InArray;            /* get the Freq Signal Input Counter value in Counting method after Filter*/ 
    ...
    else if(u8g_CurMethod == PERIODICMETHOD)                                    /* if it is period method */
    {
    	if(EMIOS_0.CH[18].CSR.B.FLAG == 1)                                      
		{
    		...
    		if(u8g_FreqInputFlag_InPodMethod == 1)
    		{
    			...
    			u32g_FreqInputDataArray[u8g_FreqInputDataArray_Counter] = ((uint32)u16g_FreqInputCount_OFC*1000000uL) + u32t_temp - u32g_FreeRunCntRegPreValue;
    			...
    			/* bubble method sorting order */
    			for(u8t_loop_cnt = FREQINPUTDATABUFFERSIZE - 1;u8t_loop_cnt > 0; u8t_loop_cnt --)
    			{		
    			   	for(uint8 u8t_loop_flag = 0; u8t_loop_flag < u8t_loop_cnt; u8t_loop_flag++)
    			   	{
   	    	...
   	    	{                        
    			...
    			for(uint8 u8t_loop_flag = 0; u8t_loop_flag < FREQINPUTDATABUFFERSIZE ; u8t_loop_flag ++)  /* initialize frequency input data buffer array */
    			{
 ...
 * Description : it used to be a timer counting overflow ISR while in PeriodMethod 
 * Arguments   : void
 * Return      : void
 */
...
{        		
    ...
    STM.CMP0.R = 50000ul;    							/* System Timer Channel 0's Compare value is : 50,000    */    
   // STM.CMP0.R = 20000ul;
...
{
	INTC.PSR[145].R = 0x08;    /* Enable eMIOS16 counting overflow Interrupt */
}
